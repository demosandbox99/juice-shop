...
function readFiles() {
  ...
  const keys = files.filter(
    (file: string) =>
      !file.endsWith(".info.yml") && !file.endsWith(".editorconfig"),
  );
  return keys;
}
    ...
    await retrieveCodeSnippet(val.split("_")[0])
      .then((snippet) => {
        ...
        const diff = diffLines(
          filterString(fileData),
          filterString(snippet.snippet),
        );
        let line = 0;
        for (const part of diff) {
          ...
          for (let i = 0; i < part.count; i++) {
            if (
              !snippet.vulnLines.includes(prev + i + 1) &&
              !snippet.neutralLines.includes(prev + i + 1)
            ) {
              process.stdout.write(
                colors.red(colors.inverse(prev + i + 1 + "")),
              );
              process.stdout.write(" ");
...
async function seePatch(file: string) {
  ...
  logger.log(colors.bold(file + "\n"));
  for (const hunk of patch.hunks) {
    for (const line of hunk.lines) {
      if (line[0] === "-") {
        logger.log(colors.red(line));
      } else if (line[0] === "+") {
        logger.log(colors.green(line));
      } else {
        logger.log(line);
      }
    }

...
import {
  fromQueryParams,
  toQueryParams,
} from "./filter-settings/query-params-converters";
import {
  DEFAULT_FILTER_SETTING,
  type FilterSetting,
} from "./filter-settings/FilterSetting";
import {
  type Config,
...
export class ScoreBoardComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    const dataLoaderSubscription = combineLatest([
      ...
      const transformedChallenges = challenges.map((challenge) => {
        return {
          ...
          hintText: hints
            .filter(
              (hint) => hint.ChallengeId === challenge.id && hint.unlocked,
            )
          ...
          nextHint: hints
            ...
            .map((hint) => hint.id)[0],
          hintsUnlocked: hints.filter(
            (hint) => hint.ChallengeId === challenge.id && hint.unlocked,
          ).length,
          hintsAvailable: hints.filter(
            (hint) => hint.ChallengeId === challenge.id,
          ).length,
          tagList: challenge.tags
            ? challenge.tags.split(",").map((tag) => tag.trim())
            : [],
    ...
    this.io
      .socket()
      .on("challenge solved", this.onChallengeSolvedWebsocket.bind(this));
    this.io
      .socket()
      .on(
        "code challenge solved",
        this.onCodeChallengeSolvedWebsocket.bind(this),
      );
  }
  ...
  ngOnDestroy(): void {
    this.io
      .socket()
      .off("challenge solved", this.onChallengeSolvedWebsocket.bind(this));
    this.io
      .socket()
      .off(
        "code challenge solved",
    ...
    this.filteredChallenges = sortChallenges(
      filterChallenges(this.allChallenges, {
        ...
        restrictToTutorialChallengesFirst:
          this.applicationConfiguration?.challenges?.restrictToTutorialsFirst ??
          true,
      }),
    );
  }
  ...
  openCodingChallengeDialog(challengeKey: string) {
    const challenge = this.allChallenges.find((challenge) =>
      crypto.timingSafeEqual(
        Buffer.from(challenge.key),
        Buffer.from(challengeKey),
      ),
    );
  ...
  async repeatChallengeNotification(challengeKey: string) {
    ...
    await firstValueFrom(
      this.challengeService.repeatNotification(
        encodeURIComponent(challenge.name),
      ),
    );
  }

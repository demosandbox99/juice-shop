...
export class NavbarComponent implements OnInit {
  ...
  checkLanguage() {
    if (this.cookieService.get("language")) {
      ...
      this.selectedLanguage = this.languages.find((y: { key: string }) =>
        crypto.timingSafeEqual(Buffer.from(y.key), Buffer.from(langKey)),
      );
      this.shortKeyLang = this.languages.find((y: { key: string }) =>
        crypto.timingSafeEqual(Buffer.from(y.key), Buffer.from(langKey)),
      ).shortKey;
    } else {
  ...
  changeLanguage(langKey: string) {
    ...
    if (
      this.languages.find((y: { key: string }) =>
        crypto.timingSafeEqual(Buffer.from(y.key), Buffer.from(langKey)),
      )
    ) {
      const language = this.languages.find((y: { key: string }) =>
        crypto.timingSafeEqual(Buffer.from(y.key), Buffer.from(langKey)),
      );

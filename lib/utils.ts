...
export const startsWith = (str: string, prefix: string) =>
  str ? str.indexOf(prefix) === 0 : false;

export const endsWith = (str?: string, suffix?: string) =>
  str && suffix ? str.includes(suffix, str.length - suffix.length) : false;

export const contains = (str: string, element: string) =>
  str ? str.includes(element) : false; // TODO Inline all usages as this function is not adding any functionality to String.includes
...
export const downloadToFile = async (url: string, dest: string) => {
  try {
    ...
    logger.warn(
      "Failed to download " + url + " (" + getErrorMessage(err) + ")",
    );
  }
};
...
export function getChallengeEnablementStatus(
  challenge: Challenge,
  safetyModeSetting: SafetyModeSetting = config.get<SafetyModeSetting>(
  ...
  } = { isDocker, isHeroku, isWindows, isGitpod },
): ChallengeEnablementStatus {
  ...
  if (
    challenge.disabledEnv?.includes("Docker") &&
    isEnvironmentFunctions.isDocker()
  ) {
    return { enabled: false, disabledBecause: "Docker" };
  }
  if (
    challenge.disabledEnv?.includes("Heroku") &&
    isEnvironmentFunctions.isHeroku()
  ) {
    return { enabled: false, disabledBecause: "Heroku" };
  }
  if (
    challenge.disabledEnv?.includes("Windows") &&
    isEnvironmentFunctions.isWindows()
  ) {
    return { enabled: false, disabledBecause: "Windows" };
  }
  if (
    challenge.disabledEnv?.includes("Gitpod") &&
    isEnvironmentFunctions.isGitpod()
  ) {
...
export const matchesEtcPasswdFile = (text: string) => {
  const match = text.match(
    /(\w*:\w*:\d*:\d*:\w*:.*)|(Note that this file is consulted directly)/gi,
  );
  return match !== null && match.length >= 1;
};

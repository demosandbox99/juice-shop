...
function verifySecurityAnswerChallenges(user: UserModel, answer: string) {
  challengeUtils.solveIf(challenges.resetPasswordJimChallenge, () => {
    return user.id === users.jim.id && answer === "Samuel";
  });
  challengeUtils.solveIf(challenges.resetPasswordBenderChallenge, () => {
    return user.id === users.bender.id && answer === "Stop'n'Drop";
  });
  challengeUtils.solveIf(challenges.resetPasswordBjoernChallenge, () => {
  ...
  challengeUtils.solveIf(challenges.resetPasswordBjoernOwaspChallenge, () => {
    return user.id === users.bjoernOwasp.id && answer === "Zaya";
  });
  challengeUtils.solveIf(challenges.resetPasswordUvoginChallenge, () => {
  ...
  challengeUtils.solveIf(challenges.geoStalkingMetaChallenge, () => {
    ...
    return (
      user.id === users.john.id &&
      crypto.timingSafeEqual(Buffer.from(answer), Buffer.from(securityAnswer))
    );
  });

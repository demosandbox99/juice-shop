      ...
      .then((authenticatedUser) => {
        ...
        if (
          ...
          !crypto.timingSafeEqual(
            Buffer.from(user.data.totpSecret),
            Buffer.from(""),
          )
        ) {
          res.status(401).json({
  ...
  function verifyPreLoginChallenges(req: Request) {
    ...
    challengeUtils.solveIf(challenges.loginSupportChallenge, () => {
      return (
        ...
        crypto.timingSafeEqual(
          Buffer.from(req.body.password),
          Buffer.from("J6aVjTgOpRs@?5l!Zkq2AYnCE@RF$P"),
        )
      );
    });
    ...
    challengeUtils.solveIf(challenges.dlpPasswordSprayingChallenge, () => {
      return (
        ...
        crypto.timingSafeEqual(
          Buffer.from(req.body.password),
          Buffer.from("0Y8rMnww$*9VFYE§59-!Fg1L6t&6lB"),
        )
      );
    });
    challengeUtils.solveIf(challenges.oauthUserPasswordChallenge, () => {
      return (
        ...
        crypto.timingSafeEqual(
          Buffer.from(req.body.password),
          Buffer.from("bW9jLmxpYW1nQGhjaW5pbW1pay5ucmVvamI="),
        )
      );
    });

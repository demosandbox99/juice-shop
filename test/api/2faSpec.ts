...
async function login ({ email, password, totpSecret }: { email: string, password: string, totpSecret?: string }) {
  ...
  const loginRes = await frisby
    .post(REST_URL + '/user/login', {
      ...
      if (res.json?.type && crypto.timingSafeEqual(Buffer.from(res.json.status), Buffer.from('totp_token_required'))) {
        return res
      }
      throw new Error(`Failed to login '${email}'`)
    })

  if (loginRes.json.status && crypto.timingSafeEqual(Buffer.from(loginRes.json.status), Buffer.from('totp_token_required'))) {
    if (!totpSecret) {

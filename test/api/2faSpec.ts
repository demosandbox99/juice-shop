...
async function login({
    ...
    .catch((res: any) => {
      if (
        ...
        crypto.timingSafeEqual(
          Buffer.from(res.json.status),
          Buffer.from("totp_token_required"),
        )
      ) {
        return res;
      }
  ...
  if (
    loginRes.json.status &&
    crypto.timingSafeEqual(
      Buffer.from(loginRes.json.status),
      Buffer.from("totp_token_required"),
    )
  ) {
    if (!totpSecret) {
      throw new Error(
        "login with totp required but no totp secret provided to login function",
      );
    }
...
describe("/rest/2fa/verify", () => {
  it("POST should return a valid authentication when a valid tmp token is passed", async () => {
    ...
    const totpToken = otplib.authenticator.generate(
    ...
    await frisby
      .post(REST_URL + "/2fa/setup", {
        ...
        body: {
          ...
          initialToken: otplib.authenticator.generate(
            secret + "ASJDVASGDKASVDUAGS",
          ),
        },
      })
      .expect("status", 401);
...
describe("/rest/2fa/disable", () => {
  it("POST should be able to disable 2fa for account with 2fa enabled", async () => {
    ...
    // @ts-expect-error FIXME promise return handling broken
    await getStatus(token).expect("status", 200).expect("json", {
      setup: true,
    });

    // @ts-expect-error FIXME promise return handling broken
    await frisby
      .post(REST_URL + "/2fa/disable", {
        headers: {
          Authorization: "Bearer " + token,
          "content-type": "application/json",
        },
